...
### `Allow USB Debugging` Dialog doesn't appear

This issue is still unknown to me but you could try these things :-

- Select `Charge Only` for the `Use Usb To` Dialog when connecting the Cable to Pc and your Phone
- go to `Settings > Additional Settigns > Developer Options`
  - scroll about 4 pages to find `Default USB configuration`
  - Open it and select `PTP`
  - Go back and Open `Select USB configuration` next to `Default USB configuration`
  - select `PTP (Picture Tranfer Protocol)`
